...
namespace ApplicationGateway.API.IntegrationTests.Controller.PolicyTest.AccessControl
{
    public class keypolicyaclquota : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        public keypolicyaclquota(CustomWebApplicationFactory factory)
        {
            _factory = factory;
        ...
        [Fact]
        public async Task CreateKeyMultiplePolicy_aclQuota()
        {
            ...
            {
                var mypolicyJsonString = File.ReadAllText(ApplicationConstants.BASE_PATH + "/PolicyData/AccessControls/key_policy_acl.json");
                ...
                foreach (var obj in keyValues["apIs"])
                {
                    obj["limit"]["quota_max"] = 3;
                    obj["limit"]["quota_renews"] = 3600;
                }
            ...
            var myKeyJsonString = File.ReadAllText(ApplicationConstants.BASE_PATH + "/PolicyData/CreatePolicyKeyAclQuota.json");
            ...
            for (var k = 0; k < apiName.Count; k++)
            {
                ...
                responseclientkeys.StatusCode.ShouldBeEquivalentTo(System.Net.HttpStatusCode.Forbidden);
            }
